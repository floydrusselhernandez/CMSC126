...
function applySorting()
{
    ...
    if (!empty($categoryFilter)) {
        ...
        if ($lastSearch && $lastSearch->num_rows > 0) {
            ...
            $searchText = $row['searchtxt'];
        
        $lastSearch->free_result();

        $sql .= " AND (p.product_name LIKE '%$searchText%' OR c.category_name LIKE '%$searchText%' OR g.genre_name LIKE '%$searchText%')";
        }
    }
    ...
    if (!empty($searchQuery)) {
        ...
        $searchSql = "INSERT INTO `search`(`search_id`, `searchtxt`, `search_date`) VALUES (NULL,'$searchQuery','ho')";
